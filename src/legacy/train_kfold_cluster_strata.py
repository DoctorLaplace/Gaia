...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Gaia Cluster-Stratified K-Fold Cross-Validation"
    )
    ...
    print(f"{Colors.HEADER}{'='*60}{Colors.ENDC}")
    print(f"{Colors.HEADER}  GAIA CLUSTER-STRATIFIED {args.k}-FOLD CROSS-VALIDATION{Colors.ENDC}")
    print(f"{Colors.HEADER}{'='*60}{Colors.ENDC}")
    ...
    print(f"\n{Colors.HEADER}{'='*60}{Colors.ENDC}")
    print(f"{Colors.HEADER}  CLUSTER-STRATIFIED {args.k}-FOLD CROSS-VALIDATION RESULTS{Colors.ENDC}")
    print(f"{Colors.HEADER}{'='*60}{Colors.ENDC}")
    ...
    # Save report
    report_path = os.path.join(project_root, "reports", "kfold_cluster_strata_results.csv")
